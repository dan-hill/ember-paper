...
moduleForComponent('o-dialog', 'Integration | Component | paper dialog', {
  integration: true
});
...
test('should render in specific wormhole if parent is defined', function(assert) {
  this.render(hbs`
    ...
    {{#o-dialog parent="#sagittarius-a"}}
      So this is singularity, eh?
    {{/o-dialog}}
  `);

  assert.ok(!$('#o-wormhole md-dialog').length, 'did not render in default');
...
test('click outside should close dialog if clickOutsideToClose', function(assert) {
  ...
  this.render(hbs`
    {{#if dialogOpen}}
      {{o-dialog clickOutsideToClose=true onClose=closeDialog}}
    {{/if}}
  `);
...
test('dialog shouldn\'t swallow click events', function(assert) {
  ...
  this.render(hbs`
    {{#o-dialog clickOutsideToClose=true}}
...
test('opening gives focus', function(assert) {
  ...
  this.render(hbs`
    {{#if showDialog}}
      {{#o-dialog onClose=closeDialog origin="#theorigin"}}
        {{#o-dialog-actions}}
...
test('can specify dialog container classes', function(assert) {
  this.render(hbs`
    {{o-dialog dialogContainerClass="flex-50 my-dialog-container"}}
  `);
...
test('can specify dialog css classes', function(assert) {
  this.render(hbs`
    {{o-dialog class="flex-50 my-dialog-inner"}}
  `);
