...
moduleForComponent('o-speed-dial', 'Integration | Component | paper speed dial', {
  integration: true
});
...
['fling', 'scale'].forEach((animation) => {
  test(`adds the correct class for ${animation} animation`, function(assert) {
    ...
    this.render(hbs`{{o-speed-dial animation=animation}}`);
...
['up', 'down', 'left', 'right'].forEach((direction) => {
  test(`adds the correct class for ${direction} direction`, function(assert) {
    ...
    this.render(hbs`{{o-speed-dial direction=direction}}`);
...
test('hoverFull=true adds the correct md-hover-full class', function(assert) {
  this.render(hbs`{{o-speed-dial hoverFull=true}}`);
...
test('focusing out the speed dial trigger closes it', async function(assert) {
  this.render(hbs`
    {{#o-speed-dial open=true as |dial|}}
      {{dial.trigger}}
    {{/o-speed-dial}}
  `);
...
test('toggling `open` opens/closes the speed dial', function(assert) {
  ...
  this.render(hbs`
    {{#o-speed-dial open=open as |dial|}}
      {{dial.trigger}}
    {{/o-speed-dial}}
  `);
...
test('clicking the trigger triggers the `onToggle` action', async function(assert) {
  ...
  this.render(hbs`
    {{#o-speed-dial onToggle=(action onToggle) as |dial|}}
      {{dial.trigger}}
    {{/o-speed-dial}}
  `);
