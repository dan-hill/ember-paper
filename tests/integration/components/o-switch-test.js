...
moduleForComponent('o-switch', 'Integration | Component | paper switch', {
  integration: true
});
...
test('should set selected class correctly', function(assert) {
  ...
  this.render(hbs`
    {{#o-switch value=switchValue onChange=foo}}
      Radio button 1
    {{/o-switch}}
  `);
...
test('should not set focused class', function(assert) {
  this.render(hbs`
    {{#o-switch value=switchValue onChange=(action (mut switchValue))}}
      Radio button 1
    {{/o-switch}}
  `);
...
test('blockless mode should render label', function(assert) {
  ...
  this.render(hbs`
    {{o-switch value=switchValue onChange=foo label="An inline label"}}
  `);
...
[32, 13].forEach((keyCode) => {
  test(`should be possible to switch on with key code ${keyCode}`, function(assert) {
    ...
    this.set('switchValue', false);
    this.render(hbs`{{o-switch value=switchValue onChange=(action (mut switchValue))}}`);
...
/* test('the `onChange` action is mandatory for o-switch', function(assert) {
  assert.expect(1);

  assert.throws(() => {
    this.render(hbs`{{o-switch value=true}}`);
