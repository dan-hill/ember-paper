...
moduleForComponent('o-autcomplete', 'Integration | Component | paper autocomplete', {
  integration: true
});
...
/* test('either `onSearchTextChange` or `onSelectionChange` functions are provided provided', function(assert) {
  ...
  assert.throws(() => {
    this.render(hbs`
      {{#o-autocomplete options=countries selected=selected as |opt|}}
        {{opt}}
      {{/o-autocomplete}}
      `);
...
test('opens on click', function(assert) {
  ...
  this.render(hbs`{{#o-autocomplete
    placeholder="Item"
    options=items
    searchText=searchText
    onSearchTextChange=(action (mut searchText))
