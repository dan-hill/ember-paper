...
moduleForComponent('o-checkbox', 'Integration | Component | paper checkbox', {
  integration: true
});
...
test('should set and remove checked css class', function(assert) {
  ...
  this.set('value', true);
  this.render(hbs`{{o-checkbox value=value label="Blue" onChange=(action (mut value))}}`);
...
test('should trigger an action when unchecking', function(assert) {
  ...
  this.render(hbs`{{o-checkbox value=value onChange=handleChange}}`);
...
test('shouldn\'t trigger an action when disabled', function(assert) {
  ...
  this.render(hbs`{{o-checkbox disabled=true value=value onChange=handleChange}}`);
...
[32, 13].forEach((keyCode) => {
  test(`should be possible to check with key code ${keyCode}`, function(assert) {
    ...
    this.set('value', false);
    this.render(hbs`{{o-checkbox value=value onChange=(action (mut value))}}`);
...
test('blockless version should set label inside', function(assert) {
  ...
  this.render(hbs`{{o-checkbox value=value onChange=(action (mut value)) label="çup?"}}`);
...
test('block version should set label inside', function(assert) {
  ...
  this.render(hbs`
    {{#o-checkbox value=value onChange=(action (mut value))}}
      çup?
    {{/o-checkbox}}
  `);
...
/* test('the `onChange` action is mandatory', function(assert) {
  ...
  assert.throws(() => {
    this.render(hbs`{{o-checkbox value=true}}`);
...
test('if `indeterminate` is true, set md-indeterminate class', function(assert) {
  ...
  this.set('value', true);
  this.render(hbs`
    {{o-checkbox value=value indeterminate=indeterminate
